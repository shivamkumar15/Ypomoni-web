...
export function Navbar() {
  ...
  useEffect(() => {
    const ids = ['features', 'how-it-works', 'evidence', 'contacts']
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 md:px-10 lg:px-12">
        ...
        <nav
          className={cn(
            'hidden items-center gap-0.5 rounded-full p-1.5 transition-all duration-500 md:flex',
        ...
        <a
          href="#download"
          className="btn-primary group hidden items-center gap-2 rounded-full bg-[#ff3f68] px-5 py-2.5 text-sm font-semibold text-black shadow-[0_0_30px_-6px_rgba(255,63,104,0.6)] transition-all duration-300 hover:scale-[1.03] hover:bg-[#ff5a7f] hover:shadow-[0_0_40px_-4px_rgba(255,63,104,0.8)] sm:inline-flex"
        >
          Get the app
        </a>
      ...
      {open && (
        <div className="mx-6 mt-2 grid gap-1 rounded-2xl glass-strong p-3 md:hidden">
          {links.map((l) => (
            <a
              key={l.href}
              href={l.href}
