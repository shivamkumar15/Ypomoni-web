      ...
      <div className="mx-auto max-w-7xl px-6 md:px-10 lg:px-12">
          ...
          {tiles.map((tile, i) => {
            ...
            return (
              <Reveal
                key={tile.title}
                delay={i * 70}
                className={`group relative flex flex-col justify-between overflow-hidden rounded-3xl glass p-7 transition-all duration-500 hover:border-[#ff3f68]/40 ${tile.span}`}
              >
                {tile.accent && (
                ...
                <div className={`relative flex h-12 w-12 items-center justify-center rounded-2xl transition-all duration-500 ${tile.accent ? 'bg-[#ff3f68] text-black' : 'bg-white/8 text-[#ff3f68]'} group-hover:scale-110`}>
                ...
                <div className="relative mt-auto pt-6">
                  ...
                  <p className="mt-3 leading-7 text-white/55">{tile.desc}</p>
                </div>
              </Reveal>
            )
          })}
        ...
        <Reveal delay={120} className="mt-8 flex flex-wrap items-center gap-4">
          {[
            ...
            const Icon = b.icon
            return (
              <div key={b.label} className="inline-flex items-center gap-2 rounded-full glass px-4 py-2.5 text-sm text-white/70">
