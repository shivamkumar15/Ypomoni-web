          ...
          <div className="relative">
            ...
            <div className="mt-10 flex items-center gap-4">
              <div className="relative h-2 w-40 overflow-hidden rounded-full bg-white/10">
                <div ref={progressRef} className="absolute inset-0 origin-top scale-y-0 bg-gradient-to-b from-[#be185d] to-[#ff3f68]" />
              </div>
              <span className="font-display text-sm font-bold text-white/60">{String(active + 1).padStart(2, '0')} / 0{steps.length}</span>
            </div>
          </div>
              ...
              return (
                <div
                  ...
                  className={`relative overflow-hidden rounded-3xl border p-7 transition-all duration-700 ${
                    isActive
                      ? 'border-[#ff3f68]/40 bg-[#ff3f68]/[0.06] scale-100 opacity-100'
                      : isPast
                ...
                >
                  {isActive && (
                    <div className="pointer-events-none absolute -right-12 -top-12 h-40 w-40 rounded-full bg-[#ff3f68]/15 blur-[60px]" />
                  )}
                  <div className="relative flex items-start gap-5">
                    <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl transition-all duration-500 ${isActive ? 'bg-[#ff3f68] text-black shadow-[0_0_30px_-6px_rgba(255,63,104,0.7)]' : 'bg-white/8 text-white/50'}`}>
                      <Icon className="h-7 w-7" />
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        <span className="font-display text-sm font-bold text-[#ff3f68]">0{i + 1}</span>
                        <h3 className="font-display text-2xl font-bold tracking-tight text-white">{step.title}</h3>
                      </div>
                      <p className="mt-3 leading-7 text-white/60">{step.copy}</p>
                      <div className={`grid transition-all duration-500 ${isActive ? 'mt-4 grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                        <div className="overflow-hidden">
                          <div className="rounded-2xl border border-white/8 bg-black/30 p-4 text-sm leading-6 text-white/50">
                            {step.detail}
                          </div>
                        </div>
                      </div>
